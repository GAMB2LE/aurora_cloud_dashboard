...
def _resolve_load_mode(
    ...
    *,
    ...
    if active_kits:
        # CL61 is sometimes powered from an AC outlet while the rest of the
        # station remains in its DC-only baseline.  Do not let the PDU
        # signature alone call that DC-only: require a sustained inverter AC
        # signal as well, so the forecast learns the correct combined load.
        if "CL61" in active_kits and current_ac_w > DEFAULT_AC_MODE_THRESHOLD_W:
