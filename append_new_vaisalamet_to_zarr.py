#!/usr/bin/env python3
"""Append Vaisala met CSV-style .dat files into a time-indexed Zarr store."""
...
def _list_files(root: Path, start_date: date | None = None) -> list[Path]:
    ...
    for path in root.glob("vaisala_met_level0_*.dat"):
...
def _read_file(path: Path) -> xr.Dataset:
    ...
    if not data_vars:
        return xr.Dataset(coords={"time": timestamps.to_numpy(dtype="datetime64[ns]")})
...
def _load_files(files: Iterable[Path], chunks: dict[str, int] | None = None) -> xr.Dataset:
    ...
    for path in files:
        ...
        try:
            ...
            print(f"Skipping unreadable Vaisala met file {path}: {exc}")
            continue
...
def append_new(
    ...
    if not root.exists():
        print(f"Raw Vaisala met directory does not exist: {root}")
        return
    ...
    if not zarr_path.exists():
        ...
        if not files:
            print("No Vaisala met .dat files available to bootstrap.")
            return
        print(f"Bootstrapping Vaisala met Zarr from {len(files)} files")
        ...
        if combined.sizes.get("time", 0) == 0:
            print("No readable Vaisala met samples available to bootstrap.")
            return
    ...
    if not files:
        print("No candidate Vaisala met .dat files to append.")
        return

    print(f"Scanning {len(files)} candidate files")
    combined = _load_files(files, chunks=None)
    if combined.sizes.get("time", 0) == 0:
        print("Candidate files contain no readable Vaisala met samples.")
        return
    combined = filter_dataset_from_time(combined, from_time)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Append Vaisala met .dat files into a Zarr store.")
