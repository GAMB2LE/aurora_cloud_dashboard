...
- **SOC Load Scenarios**
  - fixed-load what-if SOC forecasts for `100-600 W` in `100 W` steps
  - each trace uses the same latest SOC anchor and ECMWF solar forecast
- **SOC Forecast Skill**
...
The **SOC 96 h Forecast** panel is generated by
...
forecast-load legend. The model refreshes with every 15-minute learning run.
This product is operational guidance only and is stored separately from
model-evaluation products.

The **SOC Load Scenarios** panel is generated from the same ECMWF solar input
and latest `BatterySOC` anchor, but replaces the learned load model with fixed
hypothetical loads of `100`, `200`, `300`, `400`, `500`, and `600 W`. These
scenario traces are not used for adaptive learning or skill scoring.

Full refreshes download ECMWF Open Data every 3 hours. A faster 15-minute
learning refresh reuses the latest cached ECMWF GRIB, re-anchors the SOC
forecast to the latest actual `BatterySOC`, scores archived forecast runs
against newly arrived APS observations, and updates adaptive calibration state.
