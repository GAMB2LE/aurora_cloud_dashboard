...
historical UTC-hour station-load profile from `ACOutputWatts +
DCInverterWatts`, and integrates SOC forward from the latest valid
`BatterySOC`.

Before replacing the forecast, the generator scores the previous forecast
against newly arrived APS observations. It stores recent SOC, solar charging,
and load MAE plus load bias, then updates an adaptive calibration state used by
later forecast runs. The load model learns by applying a bounded bias
correction to the historical load profile: if recent load forecasts were high,
future load is reduced; if they were low, future load is increased.
...
- `ForecastLoadMAERecent` in `W`
- `ForecastLoadBiasRecent` in `W`
...
load-bias correction, and battery capacity. The product is separate from
