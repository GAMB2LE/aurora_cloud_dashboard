#!/usr/bin/env python3
"""Render power variables as stacked time-series PNGs."""
...
def plot_timeseries(ds: xr.Dataset, title: str, output: Path) -> None:
    ...
    if len(times) == 0 or not names:
        raise ValueError("No numeric power time-series variables to plot")

    height = max(8.0, min(42.0, 1.0 * len(names)))
...
def plot_last_24h(zarr_path: Path, output: Path) -> None:
    ...
    plot_timeseries(window, "power - Latest 24 hours", output)
