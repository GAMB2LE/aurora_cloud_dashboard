#!/usr/bin/env python3
"""Generate one daily stacked time-series quicklook per available power day."""
...
from plot_power_last24h import plot_timeseries
...
def main(force: bool = False) -> None:
    ...
    if force:
        for png in QUICKLOOK_DIR.glob("power_*.png"):
            png.unlink()
        print("Deleted existing power quicklook PNGs.")

    for day in dates:
        out = QUICKLOOK_DIR / f"power_{pd.Timestamp(day).strftime('%Y%m%d')}.png"
        if out.exists():
            continue
        ...
        if ds_day.sizes.get("time", 0) < 2:
            continue
        plot_timeseries(ds_day, pd.Timestamp(day).strftime("power - %Y-%m-%d"), out)


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate power daily quicklook PNGs")
