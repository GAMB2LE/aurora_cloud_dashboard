#!/usr/bin/env python3
"""Render ASFS LoggerNet variables as stacked time-series PNGs."""
...
def plot_timeseries(ds: xr.Dataset, title: str, output: Path) -> None:
    ...
    if len(times) == 0 or not names:
        raise ValueError("No numeric ASFS LoggerNet time-series variables to plot")
    ...
    fig.suptitle(title)
    fig.tight_layout()
    fig.subplots_adjust(left=0.22, bottom=0.08, top=0.96, hspace=0.18)
...
def plot_last_24h(zarr_path: Path, output: Path) -> None:
    ...
    plot_timeseries(window, "asfs-logger - Latest 24 hours", output)
