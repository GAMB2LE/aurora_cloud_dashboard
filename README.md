...
The deployed Python environment is defined by `requirements-dashboard.txt`.
Development and production hosts install this same pinned dependency set.
...
- fixed-summary interactive plots use display-time preparation and
  instrument-specific per-trace time downsampling. Power also applies sanity
  limits for impossible APS values so isolated raw outliers do not dominate the
  browser view. The Battery Charging panel applies a display-only 30-minute
  rolling mean to `BatteryAmps` and `BatteryWatts`.
- Aurora Power Supply interactive plots prefer the one-minute
  `/data/aurora/products/power/power_display_summary.zarr` product, which
  avoids repeatedly opening and merging multi-day one-second APS samples
- latest Aurora Power Supply, Meteorology, and Radiation interactive figures
  can be loaded from prewarmed Plotly JSON generated by their quicklook
  pipelines
...
- empty interactive windows include the selected UTC window and suggest the
  next useful check instead of showing a bare blank plot
...
- the dashboard opens the Power store with larger read chunks and per-trace
  time downsampling
- curated Power summary traces are precomputed into the compact display product
  `/data/aurora/products/power/power_display_summary.zarr`; this keeps the raw
  Power Zarr authoritative while avoiding multi-day one-second reads and source
  merges during interactive plotting
- `/data/aurora/products/power/power_display_energy.zarr` is still written as a
  cumulative-panel compatibility product
- the latest Power interactive figure is prewarmed as Plotly JSON under
  `/data/aurora/products/dashboard/prewarm/`; the same prewarm directory also
  holds latest Meteorology and Radiation interactive figures
