...
- `WXcam` - interactive stitched HDR video browser for `FISH HDR` and `PANO HDR`, backed by a SQLite media catalog plus an HDR image Zarr. `Interactive Data Browser` now uses a camera-style layout with obvious `Latest`, `Previous`, `Next`, and `Jump to hour` controls, a compact hourly strip, and `Updated X min ago` freshness metadata above the player. `Science Quicklooks` shows a `3 x 8` grid of hourly HDR JPG thumbnails for both today and past days, using the image nearest `:30` in each hour.
